...
def generate_archives_per_category(generator, writer):
    ...
    category_save_as = generator.settings.get('ARCHIVES_PER_CATEGORY_SAVE_AS')
    if category_save_as:
        ...
        for category, articles in generator.categories:
            ...
            save_as = category_save_as.format(category=category.slug)


            context = generator.context.copy()
            context["category"] = category
            context["categoryslug"] = category.slug
            write(save_as, generator.get_template('archives'), context,
                  dates=list(articles),
                  blog=True,
                  paginated=False)
