...
def extract_summary(instance):
    ...
    remove_summary_meta = instance.settings['SUMMARY_REMOVE_META'].lower()
    remove_summary = instance.metadata.get(remove_summary_meta, False)
    remove_markers = True
    ...
    if remove_summary:
        content = content.replace(content[begin_summary:end_summary], '')
